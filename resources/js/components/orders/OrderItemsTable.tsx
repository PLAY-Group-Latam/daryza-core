            ...
            <table className="min-w-full text-sm">
                ...
                <tbody>
                    {items.map((item) => {
                        ...
                        const regularPrice = toNumber(item.metadata?.regular_price);
                        const unitPrice = toNumber(item.unit_price);
                        const variantAttributes = (item.metadata?.variant_attributes ?? [])
                        ...
                        return (
                            <tr key={item.id} className="border-t">
                                        ...
                                        {variantAttributes.length > 0 && (
                                            <span className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
                                                {variantAttributes.map((attribute, index) => (
                                                    <span key={`${item.id}-attr-${index}`} className="inline-flex items-center gap-1">
                                                        {attribute.isColorAttribute ? (
                                                            ...
                                                            <span>{attribute.raw}</span>
                                                        )}
                                                        {index < variantAttributes.length - 1 ? (
                                ...
                                <td className="px-4 py-2">S/ {formatPen(item.line_total)}</td>
                            </tr>
                        );
                    })}
                </tbody>
