...
import { Edit, Trash, MapPin, CalendarDays, Clock  } from 'lucide-react';
        ...
        cell: ({ row }) => (
            <div className="flex items-center gap-2 text-xs">
                ...
                <div className="flex flex-col">
                    ...
                    <span className="text-[10px] text-slate-400">Hace poco</span>
                </div>
            </div>
        ),
    },
                ...
                return (
                    <div className="flex items-center gap-2">
                        <Button variant="ghost" size="icon" asChild>
                            <Link href={`/distributors/${dist.id}`}><MapPin className="h-4 w-4 text-blue-500" /></Link>
                        </Button>
                        <Button variant="outline" size="icon" asChild>
                            <Link href={`/distributors/${dist.id}/edit`}><Edit className="h-4 w-4" /></Link>
                        </Button>
                        <ConfirmDeleteAlert
                            resourceId={dist.id}
                            resourceName={dist.name}
                            routes={{
                                destroy: (id) => ({ url: `/distributors/${id}`, method: 'delete' })
                            }}
                            trigger={
    ...
    if (!data || !data.data) {
        return (
            <div className="p-4 border rounded-md  text-center">
                No se encontraron datos de distribuidores o el formato es incorrecto.
            </div>
        );
    }
