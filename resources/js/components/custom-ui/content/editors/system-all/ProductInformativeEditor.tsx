...
export default function ProductInformativeEditor({ section }: Props) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    put(`/content/update/${section.page.slug}/${section.type}/${section.id}`, {
      forceFormData: true,
      preserveScroll: true,
      onSuccess: () => toast.success('Página informativa actualizada correctamente'),
    ...
    <form onSubmit={handleSubmit} className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        <div className="px-6 py-5 border-b border-slate-100 bg-slate-50/50 flex items-center gap-3">
          ...
          <div>
            <h3 className="text-lg font-bold text-slate-900">Página Informativa del Producto</h3>
            <p className="text-sm text-slate-500">
              Imagen desktop, móvil y enlace de destino para la página informativa.
            </p>
          </div>
        </div>
