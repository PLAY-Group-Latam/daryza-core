...
import { Save, Target, FileText, Plus, Trash2 } from 'lucide-react';
...
function CardEditor({
  ...
  card:     PurposeCard;
  index:    number;
  onUpdate: (patch: Partial<PurposeCard>) => void;
  onRemove: () => void;
  canRemove: boolean;
}) {
  return (
...
export default function OurPurposeEditor({ section }: Props) {
  const rawContent = section.content?.content as OurPurposeContent;

  const { data, setData, put, processing } = useForm<{ content: OurPurposeContent }>({
    content: {
      titulo:      rawContent?.titulo      ?? '',
      descripcion: rawContent?.descripcion ?? '',
      cards:       rawContent?.cards?.length
        ? rawContent.cards
        : [DEFAULT_CARD(), DEFAULT_CARD(), DEFAULT_CARD()],
    },
  });

  const set = <K extends keyof OurPurposeContent>(key: K, val: OurPurposeContent[K]) =>
    setData('content', { ...data.content, [key]: val });

  const updateCard = (index: number, patch: Partial<PurposeCard>) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    put(`/content/update/${section.page.slug}/${section.type}/${section.id}`, {
      forceFormData: true,
      preserveScroll: true,
      onSuccess: () => toast.success('¡Nuestro Propósito actualizado!'),
        ...
        <div className="px-6 py-5 border-b border-slate-100 bg-slate-50/50">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-lg font-bold text-slate-900">Tarjetas de propósito</h3>
                <p className="text-sm text-slate-500">Imagen y nombre de cada valor.</p>
              </div>
            </div>
            <Button
              type="button"
