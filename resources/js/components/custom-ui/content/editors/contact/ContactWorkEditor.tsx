...
import { Save, ImagePlus, Monitor, Smartphone, Image, Link2, Briefcase } from 'lucide-react';
...
// ─── Editor Principal: ContactWorkEditor ───────────────────────────────────────
...
export default function ContactWorkEditor({ section }: Props) {
  ...
  const set = (key: keyof Omit<ContactWorkContent, 'banner'>, file: File | string | null) =>
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="px-6 py-5 border-b border-slate-100 bg-slate-50/50">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-bold text-slate-900">Banner "Trabaja con Nosotros"</h3>
              <p className="text-sm text-slate-500">Imagen de portada para la sección de reclutamiento.</p>
            </div>
          </div>
        </div>
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 items-start">
            <div className="space-y-2">
              <Label className="flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-widest text-slate-400">
                <Monitor size={12} /> Banner Desktop (1920x...)
              </Label>
              <UploadFixed
          ...
          {banner.type === 'url' && (
            <div className="space-y-1.5">
              <Label className="flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-widest text-slate-400">
                <Link2 size={12} /> Enlace del banner
              </Label>
              <Input
                value={banner.link_url}
                onChange={(e) => setBanner({ link_url: e.target.value })}
                placeholder="https://daryza.com/vacantes"
                className="text-sm"
              />
            </div>
          )}
      ...
      {/* ── Imagen Formulario (Reclutamiento) ── */}
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="px-6 py-5 border-b border-slate-100 bg-slate-50/50">
          <div className="flex items-center gap-3">
