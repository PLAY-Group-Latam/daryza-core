...
function normalize(raw: any): BannerContentSlides {
    if (raw?.slides && Array.isArray(raw.slides))
        return raw as BannerContentSlides;
    ...
    return (
        <div
            ...
            className={`rounded-2xl border bg-white shadow-sm transition-all select-none ${
                isDragOver
                    ? 'scale-[1.01] border-primary/50 bg-primary/5 shadow-lg'
                    : 'border-slate-200 hover:border-slate-300'
            } ${!slide.is_active ? 'opacity-50' : ''}`}
        >
            {/* ── Cabecera ── */}
            <div className="flex items-center gap-3 rounded-t-2xl border-b border-slate-100 bg-slate-50/60 px-5 py-3">
                <div className="cursor-grab text-slate-300 transition-colors hover:text-slate-500 active:cursor-grabbing">
                ...
                {canRemove && (
                    <button
                        type="button"
                        onClick={onRemove}
                        className="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-lg bg-red-50 text-red-400 transition-all hover:bg-red-500 hover:text-white"
                    >
                        <Trash2 size={13} />
                    </button>
                )}
            ...
            <div className="space-y-5 p-5">
                ...
                {slide.type === 'video' && (
                    <div className="space-y-1.5">
                        ...
                        />
                        <p className="text-[10px] text-slate-400">
                            El video se mostrará en desktop y mobile
                        </p>
                    </div>
                )}
                ...
                {slide.type === 'url' && (
                    <div className="space-y-5">
                        ...
                        <div className="space-y-1.5">
                            <Label className="flex items-center gap-1.5 text-[11px] font-semibold tracking-widest text-slate-400 uppercase">
                                <Link2 size={12} /> Enlace de redirección al
                                hacer clic
                            </Label>
                            <Input
                                value={slide.link_url}
                                onChange={(e) =>
                                    set({ link_url: e.target.value })
                                }
...
export default function BannerDinamicoEditor({ section }: ContentSectionProps) {
    ...
    const removeSlide = (i: number) => {
        if (data.content.slides.length === 1) {
            ...
            <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-slate-500">
                                Gestiona las imágenes y videos del carrusel
                                principal.
                            </p>
                        </div>
                    </div>
                ...
                <div className="space-y-4 p-6">
                    ...
                    <button
                        type="button"
                        onClick={addSlide}
                        className="flex w-full items-center justify-center gap-2 rounded-xl border-2 border-dashed border-slate-200 py-3 text-sm font-medium text-slate-400 transition-all hover:border-primary/50 hover:bg-primary/5 hover:text-primary"
                    >
